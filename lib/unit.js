 ...
 * BTC, mBTC, bits (also named uBTC) and satoshis (sats). A unit instance can be 
 ...
 * {to} method, the fixed unit methods like {toSats} or alternatively using
 * the unit accessors. It also can be converted to a fiat amount by providing the
 * corresponding BTC/fiat exchange rate.
 *
 * @example
 * ```javascript
 * var sats = Unit.fromBTC(1.3).toSats();
...
BtcUnit.fromSats = function(amount) {
  return new BtcUnit(amount, BtcUnit.sats);
};
...
BtcUnit.prototype.toSats = function() {
  return this.to(BtcUnit.sats);
};
